...
!!$  end do
  ...
  !$acc parallel num_gangs(gang_number) present(uold(:,:,:))
...
subroutine cmpdt(dt)
  ...
  ! Local variables
  integer(kind=prec_int) :: i,j
  real(kind=prec_real)   :: cournox,cournoy,eken,temp1,temp2
...
subroutine godunov(idim,dt)
 ...
 !$acc         qgdnv(1:nx+1,1:nvar) ,   flux(1:nx+1,1:nvar), n_new(1),              &
 ...
 !$acc         qleft(1:ny+1,1:nvar), qright(1:ny+1,1:nvar), n_new(1),                    &
